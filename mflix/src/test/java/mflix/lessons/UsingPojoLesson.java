...
public class UsingPojoLesson extends AbstractLesson {
   ...
   * able to successfully p values in each field to the values in the
  ...
  public void testReadObjectsWithCustomCodec() {
    ...
    Assert.assertEquals(actor1Id.toHexString(),
            actor.getId());
  ...
  public void testReadObjectWithCustomFieldCodec() {
    ...
    PropertyModelBuilder<String> idPropertyModelBuilder =
        (PropertyModelBuilder<String>) classModelBuilder
                .getProperty("id");
    ...
    ActorWithStringId stringIdActor = actors.find(queryFilter)
            .iterator().tryNext();
